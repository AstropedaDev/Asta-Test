...
smd(
  ...
  async (m, text) => {
    try {
      ...
      let apiUrl = `https://api.maher-zubair.tech/download/instagram2?url=${text}`;
      let response = await fetch(apiUrl);
      let jsonResponse = await response.json();
      ...
      if (jsonResponse.status === 200) {
        ...
        await m.send(mediaUrl, { caption: Config.caption }, "image", m);
      } else {
        await m.send("*_Request not be preceed!!_*");
      }
    } catch (error) {
...
smd(
  ...
  async (msg, query) => {
    try {
      ...
      const apiUrl = `api.maher-zubair.tech/download/tiktokimg?url=${encodeURIComponent(
        url
      )}`;
      const response = await fetch(apiUrl).then((res) => res.json());
      ...
      if (!response || response.status !== 200) {
        return await msg.reply(
          "*An error occurred while downloading the TikTok image.*"
        );
      }
...
smd(
  ...
  async (message, query) => {
    try {
      ...
      if (!input) {
        return message.reply(`*Use: ${prefix}video <video title or URL>*`);
      }

      const videoUrl = ytIdRegex.exec(input) || (await getVideoUrl(input));
      if (!videoUrl) {
      ...
      const fileType =
        info.videoDetails.lengthSeconds >= videotime ? "document" : "video";
      const videoTitle = info.videoDetails.title;

      const stream = ytdlCore(videoUrl, {
        filter: (format) => format.itag === 22 || format.itag === 18,
      });

      const filePath = `./temp/${videoTitle}.mp4`;
      ...
      await message.bot.sendMessage(message.jid, messagePayload, {
        quoted: message,
      });
  ...
  async (_0x2ee3dd, _0x20a520) => {
    try {
      ...
      if (_0x19c223.toString() == "NaN" || _0x19c223 < 1 || _0x19c223 > 160) {
        return _0x2ee3dd.reply("*_Give a number between 1 to 160_*");
      }
      let _0xf0331a =
